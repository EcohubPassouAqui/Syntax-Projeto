...
function createDiscordClient(res) {
...
app.use('/styles', express_1.default.static(path_1.default.join(process.cwd(), 'src/styles')));
app.use('/image', express_1.default.static(path_1.default.join(process.cwd(), 'src/image')));
app.use('/pages', express_1.default.static(path_1.default.join(process.cwd(), 'src/pages')));
app.get('/', (_req, res) => {
...
app.get('/api/servers', verifyToken, async (req, res) => {
    try {
        const guilds = await getGuilds(req.token);
...
function log(res, msg, level = 'info') {
    send(res, 'log', { msg, level });
}
function progress(res, text, pct, stage) {
    send(res, 'progress', { text, pct, stage });
}
async function sleep(ms) {
    return new Promise(r => setTimeout(r, ms));
}
async function buildZip(type, guild) {
    ...
    if (type === 'emojis') {
        ...
        for (const emoji of emojis) {
            try {
                const e = emoji;
                const buf = await fetch(e.url).then((r) => r.buffer());
        ...
        for (const sticker of stickers.values()) {
            try {
                const s = sticker;
                const buf = await fetch(s.url).then((r) => r.buffer());
...
async function part1_renameAndIcon(res, origin, destination) {
    ...
    try {
        ...
        if (iconUrl) {
            const buf = await fetch(iconUrl).then((r) => r.buffer());
    ...
    try {
        ...
        if (bannerUrl) {
            const buf = await fetch(bannerUrl).then((r) => r.buffer());
...
async function part2_deleteAll(res, destination) {
    ...
    for (const ch of channels) {
        try {
            await ch.delete();
            deleted++;
            log(res, `canal deletado: ${ch.name}`, 'remove');
            await sleep(300);
        }
        catch (e) {
            log(res, `erro ao deletar canal ${ch.name}: ` + e.message, 'error');
        }
    }
    log(res, `${deleted} canal(ais) deletado(s)`, 'info');
    progress(res, 'canais removidos', 48, 'removendo cargos');
    const roles = [...destination.roles.cache.values()].filter((r) => !r.managed && r.name !== '@everyone');
    for (const role of roles) {
        try {
            await role.delete();
            log(res, `cargo deletado: ${role.name}`, 'remove');
            await sleep(200);
        }
        catch (e) {
            log(res, `erro ao deletar cargo ${role.name}: ` + e.message, 'error');
        }
    }
    ...
    for (const emoji of emojis) {
        try {
            await emoji.delete();
            log(res, `emoji deletado: ${emoji.name}`, 'remove');
            await sleep(200);
        }
        catch (e) {
            log(res, `erro ao deletar emoji ${emoji.name}: ` + e.message, 'error');
        }
    }
    ...
    try {
        ...
        for (const sticker of stickers.values()) {
            try {
                ...
                log(res, `figurinha deletada: ${sticker.name}`, 'remove');
                await sleep(300);
            }
            catch (e) {
                log(res, `erro ao deletar figurinha ${sticker.name}: ` + e.message, 'error');
            }
        }
...
function mapOverwrites(res, ch, roleIdMap) {
    const overwrites = [];
...
async function part3_cloneContent(res, origin, destination) {
    ...
    const allChannels = [...origin.channels.cache.values()]
        .filter((ch) => !threadTypes.includes(ch.type))
    ...
    const textVoice = allChannels.filter((ch) => ch.type !== 'GUILD_CATEGORY');
    ...
    for (const emoji of emojis) {
        try {
            const buf = await fetch(emoji.url).then((r) => r.buffer());
            await destination.emojis.create(buf, emoji.name);
            log(res, `emoji copiado: ${emoji.name}`, 'add');
            await sleep(300);
        }
        catch (e) {
            log(res, `erro ao copiar emoji ${emoji.name}: ` + e.message, 'error');
        }
    }
    ...
    try {
        ...
        for (const sticker of stickers.values()) {
            try {
                ...
                log(res, `erro ao copiar figurinha ${sticker.name}: ` + e.message, 'error');
            }
        }
...
app.post('/api/clone', verifyToken, async (req, res) => {
    ...
    try {
        ...
        if (!botMember.permissions.has(discord_js_selfbot_v13_1.Permissions.FLAGS.MANAGE_CHANNELS) ||
            !botMember.permissions.has(discord_js_selfbot_v13_1.Permissions.FLAGS.MANAGE_ROLES) ||
            !botMember.permissions.has(discord_js_selfbot_v13_1.Permissions.FLAGS.MANAGE_EMOJIS_AND_STICKERS)) {
...
app.post('/api/webhook-zip', verifyToken, async (req, res) => {
    const { serverId, type, webhookUrl } = req.body;
...
app.post('/api/download-zip', verifyToken, async (req, res) => {
